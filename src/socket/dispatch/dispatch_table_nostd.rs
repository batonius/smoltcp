...
impl DispatchTable {
    ...
    pub(crate) fn add_socket(&mut self, _socket: &Socket, _handle: SocketHandle)
                             -> Result<(), Error> {
        Ok(())
    }

    #[allow(dead_code)]
    pub(crate) fn add_udp_socket(&mut self, _udp_socket: &UdpSocket, _handle: SocketHandle)
                                 -> Result<(), Error> {
        Ok(())
    }

    #[allow(dead_code)]
    pub(crate) fn add_raw_socket(&mut self, _raw_socket: &RawSocket, _handle: SocketHandle)
                                 -> Result<(), Error> {
        Ok(())
    }

    #[allow(dead_code)]
    pub(crate) fn add_tcp_socket(&mut self, _tcp_socket: &TcpSocket, _handle: SocketHandle)
                                 -> Result<(), Error> {
        Ok(())
    }

    pub(crate) fn remove_socket(&mut self, _socket: &Socket, _handle: SocketHandle)
                                -> Result<(), Error> {
        Ok(())
    }
